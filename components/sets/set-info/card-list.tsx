...
export const getCardTypeColor = (type?: string, subtype?: string) => {
    if (!type && !subtype) return "bg-gray-500"
    const t = type?.toLowerCase() || ""
    const sub = subtype?.toLowerCase() || ""

    if (t.includes("spell")) return "bg-[#06b6d4]"
    if (t.includes("trap")) return "bg-[#db2777]"
    if (t.includes("monster")) {
        if (sub.includes("normal")) return "bg-[#fde68a]"
        return "bg-[#c58a5c]"
    }

    if (t.includes("extra")) {
        if (sub.includes("xyz")) return "bg-black"
        if (sub.includes("fusion")) return "bg-[#a855f7]"
        if (sub.includes("synchro")) return "bg-[#e2e8f0]"
    }
                ...
                {set.cards.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-1 pr-2">
                        {set.cards.map((card) => (
                            <div
                                key={card.id}
                                className="flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm bg-muted/30 hover:bg-muted/60 transition-colors group"
                            >
